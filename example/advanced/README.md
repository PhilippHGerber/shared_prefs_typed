...
- **Public schema class** — `AppPreferences` (no leading `_`) generates `AppPreferencesImpl`
  and `AppPreferencesBase` (the abstract interface)
...
| `lib/app_preferences.g.dart` | Generated — `AppPreferencesImpl` + `AppPreferencesBase` |
| `lib/service_locator.dart` | Registers `AppPreferencesImpl` under `AppPreferencesBase` in `get_it` |
| `lib/main.dart` | Calls `setupLocator()` before `runApp`, reads prefs via `getIt<AppPreferencesBase>()` |
